...
- The addressee is one flush-left stack with no gaps: `Mr./Mrs./Miss.`, the name lines, the address lines, and the holder's e-TIN (`UNIT_KYC.ETIN1`) as the last line. Particulars headers centre each wrapped line.
- Body paragraph and notes are justified by `ComposeJustifiedParagraph`, ported from UFMS unchanged: words are measured with SkiaSharp 2.88 (the version UFMS renders with — the `SkiaSharp` package reference exists only for this), broken into lines against the content width, and each full line laid out as a row of words with equal gaps. Do not swap it for QuestPDF's own `Justify()`: that breaks lines a little later, and the emailed certificate must wrap exactly where the UFMS one does. Known residual difference: QuestPDF 2025 wraps the "Tax Deduction @Tk. n%" header as "Tax Deduction / @Tk. n%" where QuestPDF 2022 in UFMS gives "Tax Deduction @Tk. / n%", and its line spacing is marginally tighter, so a full 12-challan certificate needs no scale-to-fit here while it does in UFMS.
